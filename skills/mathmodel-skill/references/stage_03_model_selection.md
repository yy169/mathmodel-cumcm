...
loads_reference:
  ...
  - "references/model_route_confirmation.md"
...
读取 `decision_log.stages.2.literature_bootstrap` 和
`decision_log.stages.2.literature_quality_gate`：

- `pass`：正常进入候选模型比较；
- `conditional`：可以生成候选和 toy demo，但缺失证据相关的参数、机制或创新主张必须
  保持待定，不能锁定为终稿结论；
- `block`：停止最终模型选择，先完成自动检索或向用户索取明确列出的关键资料。

不得因为用户没有上传优秀论文就跳过检索，也不得因为用户上传了优秀论文就跳过原始来源
核验。候选模型的“文献或理论支持”必须定位到文献—主张映射。
...
读取 `model_route_confirmation.md`，生成 `reports/model_route_checkpoint.md` 并主动向用户
汇报题目理解、数据条件、检索关键词、高质量文献链接、候选模型矩阵、推荐和否决理由、
预期创新、求解/验证计划及风险。

写入 `decision_log.stages.3.human_model_checkpoint`，默认状态为 `pending`。在用户明确回复
批准前，不得执行 Step 4 之后的最终模型命名、主体求解或论文写作。用户要求调整时状态为
`revision_requested`，只修订受影响候选后重新确认。
...
### Step 4.1: 人为参数——外生给定还是从模型解出 (10 min)

列出所有**题面未给定、但会改变答案的参数**（风险水平、质量点、阈值、权重、先验超参、
惩罚系数）。逐项登记到 `decision_log.stages.3.artificial_parameters`：

| 字段 | 含义 |
|---|---|
| `symbol` | 符号 |
| `status` | `exogenous`（外生给定）或 `derived`（从模型解出） |
| `justification` | 若 exogenous，**为什么不能解出**；若 derived，从什么准则解出 |
| `demo_value` | 演示取值 |
| `sensitivity_artifact` | 展示该取值影响的结果文件或图 |

**关键要求：选择 `exogenous` 时必须回答"为什么不能解出"，而不只是"我取了这个值"。**

可接受的理由例如：该参数表达企业风险偏好，题面未给损失口径，任何取值都需外部输入；
不可接受的理由是：没想过能不能解。

**为什么在 Stage 3 问**：把一个参数从外生改成解出，要新建一层准则（损失函数、效用、
风险预算）并接进现有模型，是**结构性改动**。Stage 3 决定还来得及，终稿阶段只能补
敏感性表，补不了依据。

2024B 的 $p_1$（不可接受质量点）是典型：论文取 0.20 作演示、给出了完整的样本量敏感性、
并明说"企业应依据损失容忍度重新给定"——**处理是诚实的，但把问题退回给了提问者**。
若在 Stage 3 就把检测成本与漏检损失写成一个准则，$p_1$ 便可从中解出，0.20 就不再是
演示值而是结果。该项也正是终稿适用边界表中列出的扩展方向，写在那里意味着当时已经
来不及做。

评审对"这个数为什么是这个数"的追问，只能在这一步预先解决。
...
3. 文献检索门禁和质量门禁均不是 `block`，且选型主张能定位到文献—主张映射；
   外部参数能定位到来源台账，关键冲突已处理
3.1 `stages.3.artificial_parameters` 已登记全部题面未给定却影响答案的参数；
   每个 `exogenous` 项都写明了**为什么不能解出**，而不只是取值
4. `human_model_checkpoint.status == approved`，且推荐路线、基线和验证计划已记录
5. toy demo 通过
