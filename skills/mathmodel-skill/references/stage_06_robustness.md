...
| 自建数值内核 | **独立方法对拍**（原理不同的第二套算法重算同一量） | 内核承担核心数值，且无解析解可校验 |

LHS 适合有依据的连续多参数域，OAT 可处理局部单参数风险，Sobol 适合样本预算充足的方差归因。三者没有等级顺序；选择理由写入 `decision_log.stages.6.method`。

**内部自洽 ≠ 正确** ⭐

两类验证能证明的事情不同，论文里也要分开写：

| 类型 | 能证明 | 不能证明 |
|---|---|---|
| 内部自洽检验（收敛研究、极限对拍、守恒检查、不动点、单调性） | 代码做了设计者想做的事 | 设计本身符合题意 |
| 独立方法对拍（原理不同的第二套算法） | 收敛到的值是对的 | —— |

**求积/离散收敛检验不能替代独立方法对拍**：收敛曲线只能说明“细化后不再变”，
说明不了“**收敛到的值偏了**”。若项目自建了承担核心数值的内核，对最重的 1–2 个分量
各用一套原理不同的算法重算，报告逐例差值的 RMS、符号分布与区间。

差值若呈**同号集中**（例如 35/36 同号，符号检验 `p≈1e-9`），那是**系统偏差**不是随机误差，
必须定位机理。**发现系统偏差要如实报告，不要修掉不提**——即便它比所有模型假设的影响
都小一个数量级、不改变任何结论，报告它本身就是验证体系有效的证据。

同时**分两层标注精度**：区分“给定模型下的数值解，数值误差 `10⁻⁴`”与“模型不确定性 `10⁻²`”；
结论解释中不得暗示末位小数有工程意义。

详见 `references/optimization_search_rigor.md` §12。
...
**抽样代理的权重偏差（跨方案比较时禁用代理）** ⭐

为省算力而自制的分箱/代表点抽样，会在箱内个体数不均衡时引入系统性权重偏差。该偏差对
不同方案**不等量**，因此跨方案比较可能失效甚至**符号反转**（实测案例：分箱探针给出
方案 B 比 A 差 0.903%，全样本给出 +0.567%）。

- 任何分箱代理，先检查**箱内个体数是否均衡**；不均衡则按箱内个数加权，或直接用全样本；
- **跨方案比较一律用全样本**，代理只允许用于单方案粗筛；
- 敏感性结论用**区间表述**，不要从多个设定里挑一个端点命名为“最保守”——
  方案之比的变化与设定的保守程度之间通常**没有单调关系**。

详见 `references/optimization_search_rigor.md` §12.1。
...
- F5：用收敛研究充当正确性证据 → 补独立方法对拍；收敛只说明“不再变”，不说明“值对”
- F6：跨方案比较用了自制分箱/代表点抽样 → 改全样本；箱内个体数不均衡会使比较符号反转
