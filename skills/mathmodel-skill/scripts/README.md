...
### `sanity_check.py` — 物理量合理性检查（Stage 2 Step 0）

拿到附件后第一个跑。根据表头里的词和单位判断每列的物理含义，报告讲不通的数值；同时检查自变量轴是否单调等间隔、多个附件的轴能否直接合并。

```bash
python scripts/sanity_check.py "/path/to/附件/*.xlsx" -o /path/to/project/state/sanity
python scripts/sanity_check.py /path/to/附件1.xlsx /path/to/附件2.csv -o /path/to/project/state/sanity
```

只报告不改数。产出 `sanity_report.md`（含"为什么这个值不可能"的依据）和 `violations.csv`。每条发现都要在项目里留下"处理方式 + 理由"，最终进论文的数据预处理小节。

对量纲已经是小数的比例列（最大值 ≤ 1.5）不按百分数判定，避免误报。

### `audit_data.py` — 多表结构体检（Stage 2 Step 0，复杂附件时补跑）

适用于同一主体多次出现、跨表主键、大量缺失的业务型数据（典型如国赛 C 题）。检查列名、类型、缺失、重复测量导致的独立性破坏、个体内本应恒定字段的前后矛盾、跨表同名列类型冲突。

传 `-p 题面` 时额外做**题面-数据对账**：把题面里写死的区间和阈值抓出来，和附件实际取值比对，报告"题面说的范围和数据给的对不上"。对不上的地方往往是论文里的加分点。

```bash
python scripts/audit_data.py /path/to/附件.xlsx -p /path/to/题面.pdf -o /path/to/project/state/audit
```

产出 `data_audit_report.md`、`data_decision_log.csv`（`处理方式`/`理由` 两列待填）和缺失分布图。题面为扫描件时先用 pymupdf 导出文本再传 `.txt`。

两表以内、列数很少的科学测量数据不必跑这个，`sanity_check.py` 已经够。
...
从 `decision_log.compliance.ai_usage` 生成竞赛要求的披露材料，并直接放到渲染器约定的位置。CUMCM 使用 AI 时输出 `paper_workspace/AI工具使用声明.md` 与 `support_materials/AI工具使用详情.{md,pdf}`；显式未使用时只输出 `paper_workspace/AI工具未使用声明.md`。两种声明互斥，渲染器都接在参考文献**前**（《人工智能工具使用规定（2026 年试行）》第 3 条），措辞按规定原文固定，不得改写。MCM 输出 `paper_workspace/11_ai_use_report.md`，且不重复模板提供的标题。CUMCM 的 PDF 生成依赖 ReportLab。
...
## `workflow_manager.py` — 中央阶段状态与增量重跑

```bash
python scripts/workflow_manager.py --project /path/to/project status
python scripts/workflow_manager.py --project /path/to/project validate --stage 3
python scripts/workflow_manager.py --project /path/to/project checkpoint --stage 3
python scripts/workflow_manager.py --project /path/to/project \
  plan-rerun --changed code/model.py paper/main.tex --write
```

manifest只负责执行契约；模型理由仍写入`decision_log.json`。

## `build_code_manifest.py` — 代码清单与自动代码附录

```bash
python scripts/build_code_manifest.py --project /path/to/project
python scripts/build_code_manifest.py --project /path/to/project --check
python scripts/build_code_manifest.py --project /path/to/project --include-source
```

默认生成`results/code_manifest.json`与`paper_workspace/code_appendix.md`。
