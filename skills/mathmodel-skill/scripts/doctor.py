...
# CUMCM has no top-level 检验/稳健性 chapter: that content lives inside 5_models as
# 5.X.X subsections (competitions/cumcm/paper_writing.md). MCM and Diangong keep it.
EXPECTED_RENDER_MARKERS = {
    "cumcm": (CORE_SECTION_MARKERS - {"6_sensitivity"})
...
def run_checks(
    ...
    json_paths = [
        ...
        SKILL_ROOT / "assets" / "workflow_manifest.json",
    ]
    ...
    decision_schema_ok = (
        ...
        and isinstance(decision.get("execution"), dict)
    )
    checks.append(_check(
        "decision-log-schema",
        decision_schema_ok,
        "decision_log schema 3.2 with compliance and execution state"
    ...
    workflow_path = SKILL_ROOT / "assets" / "workflow_manifest.json"
    workflow = parsed.get(workflow_path, {})
    workflow_ok = (
        isinstance(workflow, dict)
        and workflow.get("_schema_version") == "1.0"
        and workflow.get("decision_log_schema") == "3.2"
        and list(workflow.get("stage_order", [])) == list(range(10))
        and isinstance(workflow.get("stages"), dict)
        and len(workflow.get("stages", {})) == 10
        and isinstance(workflow.get("change_impact_rules"), list)
    )
    checks.append(_check(
        "workflow-manifest",
        workflow_ok,
        "10-stage workflow manifest parsed"
        if workflow_ok else "workflow manifest is incomplete or incompatible",
    ))
