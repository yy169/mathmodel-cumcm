...
Outputs are placed where the paper renderer expects them:
  * cumcm with AI use: ``paper_workspace/AI工具使用声明.md`` (the in-paper
    declaration, rendered immediately before the references) plus
    ``support_materials/AI工具使用详情.{md,pdf}``
  * cumcm without AI use: ``paper_workspace/AI工具未使用声明.md``
  * mcm: ``paper_workspace/11_ai_use_report.md``

《全国大学生数学建模竞赛人工智能工具使用规定（2026 年试行）》第 3 条 requires
exactly one declaration, placed before the references, in one of two fixed
wordings; 第 4 条 additionally requires the details PDF when AI was used.
...
def render_cumcm_no_use_statement() -> str:
    """Return the no-AI declaration that must appear immediately before references.

    Wording is fixed by 《全国大学生数学建模竞赛人工智能工具使用规定（2026 年试行）》
    第 3 条 (1); do not paraphrase it.
    """
    return "本参赛队在竞赛过程中未使用任何 AI 工具。\n"


def render_cumcm_use_statement(entries: Sequence[AIUsageEntry]) -> str:
    """Return the AI-use declaration that must appear immediately before references.

    Wording is fixed by 《全国大学生数学建模竞赛人工智能工具使用规定（2026 年试行）》
    第 3 条 (2). Only the bracketed purpose summary is team-supplied; it is derived
    here from the ledger so it cannot silently disagree with 《AI工具使用详情.pdf》.
    """
    purposes: List[str] = []
    ...
    if not purposes:
        raise LedgerValidationError(
            "compliance.ai_usage 非空时每条都必须填写 'purpose'，"
            "否则无法生成规定第 3 条 (2) 要求的用途摘要"
        )
    return (
        "本参赛队在竞赛过程中使用了 AI 工具，主要用于"
        f"{'、'.join(purposes)}，详细使用情况见支撑材料。\n"
    )
...
def render_reports(
    ...
    *,
    ...
    statement_path.write_text(
        render_cumcm_use_statement(entries), encoding="utf-8"
    )
