...
# Sections a template is allowed to omit entirely.
# `6_sensitivity`: CUMCM forbids a top-level 检验/稳健性 chapter — that content belongs
# inside chapter 5 as `5.X.X` subsections. MCM/Diangong templates still carry the marker,
# so they keep requiring 07_sensitivity.md.
TEMPLATE_DROPPABLE_SECTIONS = {"6_sensitivity"}
...
def validate_workspace_sections(workspace: Path, required_sections=None) -> None:
    """Require each workspace input the template actually asks for to exist and be non-empty.

    `required_sections` is the set of MATHMODEL section markers found in the template.
    Passing None keeps the historical behaviour of requiring all of `SECTION_TO_FILE`.
    CUMCM templates omit `6_sensitivity` on purpose — model checking and robustness are
    written inside chapter 5 as `5.X.X` subsections, never as a top-level chapter
    (see competitions/cumcm/paper_writing.md, which outranks this script).
    """
    if required_sections is None:
        wanted = dict(SECTION_TO_FILE)
    else:
        wanted = {k: v for k, v in SECTION_TO_FILE.items() if k in required_sections}
    missing = []
    empty = []
...
def fill_template_main(
    ...
    # Which workspace files are required is decided by the template, not by a fixed list:
    # a competition whose template drops a section marker must not be forced to ship
    # that section's markdown file.
    validate_workspace_sections(
        workspace,
        set(SECTION_MARKER_RE.findall(main_src.read_text(encoding="utf-8"))),
    )
    ...
    missing_markers = sorted(set(SECTION_TO_FILE) - set(markers) - TEMPLATE_DROPPABLE_SECTIONS)
    if missing_markers:
        raise ValueError(
    ...
    section_files = {k: v for k, v in SECTION_TO_FILE.items() if k in markers}
