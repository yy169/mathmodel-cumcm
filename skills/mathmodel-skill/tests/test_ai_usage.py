...
class AIUsageReportTests(unittest.TestCase):
    ...
    def test_cumcm_markdown_and_pdf_are_generated(self) -> None:
        ...
        # 第 3 条 (2): the in-paper declaration uses the prescribed wording and
        # carries a purpose summary drawn from the ledger.
        statement = outputs[0].read_text(encoding="utf-8")
        self.assertTrue(
            statement.startswith("本参赛队在竞赛过程中使用了 AI 工具，主要用于")
        )
        self.assertTrue(statement.endswith("详细使用情况见支撑材料。\n"))
        self.assertIn(
    ...
    def test_cumcm_empty_ledger_generates_inline_statement_only(self) -> None:
        ...
        # 第 3 条 (1): wording is prescribed verbatim, including 「在竞赛过程中」.
        self.assertEqual(
        ...
        self.assertFalse((paper_workspace / "AI工具使用声明.md").exists())
